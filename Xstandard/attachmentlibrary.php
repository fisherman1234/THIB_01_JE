...
define("XS_BASE_URL", "http://" . $_SERVER["SERVER_NAME"] . substr($_SERVER["PHP_SELF"], 0, strrpos($_SERVER["PHP_SELF"], "/") + 1)); // Base URL to create for files. Relative URLs are okay, for example: "docs/".
define("XS_CONFIG_FILE", "attachmentlibrary.config"); // Path to config file.
...
define("XS_UPLOAD_OBJECT_MAX_SIZE", 2048000); //Maximum file upload size.
	...
	if (array_key_exists("/soap:Envelope[1]/soap:Body[1]/doLibraryDescribe[1]", $request)) {
		...
		*/
		...
		foreach ($file_list as $key => $fs_object) {
			$hidden = false;
			$label = $fs_object;
			$item = xs_get_config_object($config, "attachment", xs_build_path($path, $fs_object));
			$icon = "";
			$attributes = array();
			$properties = array();
			if (is_array($item)) {
				...
				if ($item["newWindow"] == "yes") {
					array_push($properties, array("name" => "newWindow", "value" => "true"));
				}
			}
			...
			if (!$hidden) {
				...
				if ($using_new_base_url) {
					array_push($attributes, array("name" => "href", "value" => xs_build_path($base_url, xs_urlencode($fs_object))));
				} else {
					array_push($attributes, array("name" => "href", "value" => xs_build_path(xs_build_path($base_url, xs_urlencode($path)), xs_urlencode($fs_object))));
				}
		...
		** -------------------------------------------------------
		*/		
		array_push($attributes, array("name" => "href", "value" => "http://xstandard.com/images/logo.gif"));
