...
** - PHP file and config file is set to Everyone.
*************************************** IMPORTANT **************************************/


/*************************** OPTIONAL - CHANGE THESE SETTINGS **************************/
define("XS_DIRECTORY_NAME", "Directory Service"); // Name of the service
define("XS_CONFIG_FILE", "directory.config"); // Path to config file.
...
define("XS_BROWSE_ENABLED", true); // Enable the ablity to browse this Directory.
define("XS_SEARCH_ENABLED", false); // Enable the ablity to search this Directory. You'll need to customize the search feature to meet your CMS needs.
...
class XS_Directory_Config_File {
	var $xpath = "";
	var $xpath_stack = array();
	var $rules = array();
	var $rule = array("id" => "", "script" => "");
	...
	function startElement($parser, $name, $attributes) {
		...
		if ($this->xpath == "/directory/rule") {
			$this->dictionary["id"] = "";
			$this->dictionary["script"] = "";
		}
	}
	...
	function endElement($parser, $name) {
		// Get the data
		if ($this->xpath == "/directory/rule") {
			array_push($this->rules, $this->rule);
		}
	...
	function charData($parser, $text) {
		...
		switch ($this->xpath) {
		case "/directory/rule/id":
			$this->rule["id"] = $text;
			break;
		case "/directory/rule/script":
			$this->rule["script"] = $text;
			break;
		}
	}
}
...
class XS_Directory {
	var $dir_containers = array();
	var $dir_objects = array();
	var $id = "";
	var $metadata = "";
	var $lang = "en";
	
	function XS_Directory() {
		$this->dir_containers = array();
		$this->dir_objects = array();
		$this->lang = "en";
	}
	
	function add_container($label, $id, $metadata = "", $location = "", $icon = "") {
		$GLOBALS["order"] = $GLOBALS["order"] + 1;
		array_push($this->dir_containers, array("label" => $label, "id" => $id, "metadata" => $metadata, "location" => $location, "icon" => $icon, "order" => $GLOBALS["order"]));
	}
	
	function add_object($label, $data, $icon = "") {
		$GLOBALS["order"] = $GLOBALS["order"] + 1;
		array_push($this->dir_objects, array("label" => $label, "data" => $data, "icon" => $icon, "order" => $GLOBALS["order"]));
	}
	...
	function log_to_file($msg) {
		xs_debug_to_file($msg);
	}
	
	function current_folder() {
		$path = str_replace("\\", "/", stripslashes($_SERVER["PATH_TRANSLATED"]));
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$directory = new XS_Directory();
	$xml_config = new XS_Directory_Config_File();
	$config = array();
	$request = $xml_parser->parse($soap);

	// Determine what to do
	if (array_key_exists("/soap:Envelope[1]/soap:Body[1]/doDirectoryDescribe[1]", $request)) {
		...
		echo "<soap:Envelope xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:soap=\"" . $soap_namespace_uri . "\">";
			echo "<soap:Body>";
    				echo "<doDirectoryDescribeResponse xmlns=\"http://xstandard.com/2004/web-services\">";
					echo "<doDirectoryDescribeResult>";
						echo "<directory>";
							echo "<name>" . xs_xml_escape(XS_DIRECTORY_NAME) . "</name>";
							echo "<description>string</description>";
						echo "</directory>";
						...
						echo "<directorySearch>";
							...
							echo "</enabled>";
						echo "</directorySearch>";
						echo "<directorySearchFilters>";
						foreach ($search_filters as $key => $item) {
							echo "<directorySearchFilter>";
						...
						echo "</directorySearchFilters>";
					echo "</doDirectoryDescribeResult>";
				echo "</doDirectoryDescribeResponse>";
  			echo "</soap:Body>";
		echo "</soap:Envelope>";
	} elseif (array_key_exists("/soap:Envelope[1]/soap:Body[1]/doDirectoryBrowse[1]", $request)) {
		// Browse directory
		
		if (!XS_BROWSE_ENABLED) {
			xs_debug_to_file("Browse feature is not enabled.");
			xs_respond_error("Browse feature is not enabled.", $soap12);
			exit(0);
		}

		$id = $request["/soap:Envelope[1]/soap:Body[1]/doDirectoryBrowse[1]/id[1]"];
		$metadata = $request["/soap:Envelope[1]/soap:Body[1]/doDirectoryBrowse[1]/metadata[1]"];
		$lang = $request["/soap:Envelope[1]/soap:Body[1]/doDirectoryBrowse[1]/lang[1]"];
		$script = "";
		$found = false;
		...
		if ($found === false) {
			xs_debug_to_file("The following rule was not found in config file: " . $id);
			xs_respond_error("Cannot find Directory rule: " . $id, $soap12);
			exit(0);
		}
		...
		if(strlen($script) > 0) {
			if(@eval($script) === false) {
				xs_debug_to_file("Error parsing or executing script for rule: " . $id);
				xs_respond_error("Error processing rule: " . $id, $soap12);
				exit(0);
			}
		} else {
			xs_debug_to_file("There is no script defined for rule: " . $id);
			xs_respond_error("Error processing rule: " . $id, $soap12);
			exit(0);
		}
		...
		echo "<soap:Envelope xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:soap=\"" . $soap_namespace_uri . "\">";
			echo "<soap:Body>";
				echo "<doDirectoryBrowseResponse xmlns=\"http://xstandard.com/2004/web-services\">";
					echo "<doDirectoryBrowseResult>";
						echo "<dirContainers>";
						foreach ($directory->dir_containers as $key => $item) {
							echo "<dirContainer>";
								echo "<label>" . xs_xml_escape($item["label"]) . "</label>";
								echo "<id>" . xs_xml_escape($item["id"]) . "</id>";
								echo "<metadata>" . xs_xml_escape($item["metadata"]) . "</metadata>";
								echo "<location>" . xs_xml_escape($item["location"]) . "</location>";
								echo "<icon>" . xs_xml_escape($item["icon"]) . "</icon>";
								echo "<order>" . xs_xml_escape($item["order"]) . "</order>";
							echo "</dirContainer>";
						}
						echo "</dirContainers>";
						echo "<dirObjects>";
						foreach ($directory->dir_objects as $key => $item) {					
							echo "<dirObject>";
								echo "<label>" . xs_xml_escape($item["label"]) . "</label>";
								echo "<data>" . xs_xml_escape($item["data"]) . "</data>";
								echo "<icon>" . xs_xml_escape($item["icon"]) . "</icon>";
								echo "<order>" . xs_xml_escape($item["order"]) . "</order>";
							echo "</dirObject>";
						}
						echo "</dirObjects>";
					echo "</doDirectoryBrowseResult>";
				echo "</doDirectoryBrowseResponse>";
			echo "</soap:Body>";
		echo "</soap:Envelope>";
		$GLOBALS["order"] = 0;
	} elseif (array_key_exists("/soap:Envelope[1]/soap:Body[1]/doDirectorySearch[1]", $request)) {
		// Search directory
		
		if (!XS_SEARCH_ENABLED) {
			xs_debug_to_file("Search feature is not enabled.");
			xs_respond_error("Search feature is not enabled.", $soap12);
			exit(0);
		}
		
		$search_for = $request["/soap:Envelope[1]/soap:Body[1]/doDirectorySearch[1]/searchFor[1]"];
		$filter_by = $request["/soap:Envelope[1]/soap:Body[1]/doDirectorySearch[1]/filterBy[1]"];
		$lang = $request["/soap:Envelope[1]/soap:Body[1]/doDirectorySearch[1]/lang[1]"];
		$dir_objects = array();

	
		/*************************** OPTIONAL - CHANGE THESE SETTINGS **************************/
		array_push($dir_objects, array("label" => "Test 1", "data" => "<p>Hello World</p>", "icon" => ""));
		array_push($dir_objects, array("label" => "Test 2", "data" => "<p>Hello World</p>", "icon" => ""));
		array_push($dir_objects, array("label" => "Test 3", "data" => "<p>Hello World</p>", "icon" => ""));
		array_push($dir_objects, array("label" => "Test 4", "data" => "<p>Hello World</p>", "icon" => ""));
		array_push($dir_objects, array("label" => "Test 5", "data" => "<p>Hello World</p>", "icon" => ""));
		array_push($dir_objects, array("label" => "Test 6", "data" => "<p>Hello World</p>", "icon" => ""));
		array_push($dir_objects, array("label" => "Test 7", "data" => "<p>Hello World</p>", "icon" => ""));
		array_push($dir_objects, array("label" => "Test 8", "data" => "<p>Hello World</p>", "icon" => ""));
		array_push($dir_objects, array("label" => "Test 9", "data" => "<p>Hello World</p>", "icon" => ""));
		array_push($dir_objects, array("label" => "Test 10", "data" => "<p>Hello World</p>", "icon" => ""));
		/*************************** OPTIONAL - CHANGE THESE SETTINGS **************************/
		...
		echo "<soap:Envelope xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:soap=\"" . $soap_namespace_uri . "\">";
			echo "<soap:Body>";
				echo "<doDirectorySearchResponse xmlns=\"http://xstandard.com/2004/web-services\">";
					echo "<doDirectorySearchResult>";
						echo "<dirObjects>";
						foreach ($dir_objects as $key => $item) {					
							echo "<dirObject>";
								echo "<label>" . xs_xml_escape($item["label"]) . "</label>";
								echo "<data>" . xs_xml_escape($item["data"]) . "</data>";
								echo "<icon>" . xs_xml_escape($item["icon"]) . "</icon>";
						...
						echo "</dirObjects>";
					echo "</doDirectorySearchResult>";
				echo "</doDirectorySearchResponse>";
			echo "</soap:Body>";
		echo "</soap:Envelope>";
		$GLOBALS["order"] = 0;
	} else {
